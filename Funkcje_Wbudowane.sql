--****Tworzenie tabeli regions_train na podstawie ju¿ istniejacej tabeli regions****
CREATE TABLE regions_train
    AS
        SELECT
            *
        FROM
...
--****Tworzenie tabeli jobs_train na podstawie ju¿ istniejacej tabeli jobs****
CREATE TABLE jobs_train
    AS
        SELECT
            *
        FROM
...
--****Dodawanie rekordów z wykorzystaniem sekwencji na polu ID****; 
...
SELECT
    ...
    CASE
        ...
        WHEN salary >= 3000
            ...
            '3k-6k'
        WHEN salary >= 6000
             AND salary <= 9000 THEN
            '6k-9k'
        WHEN salary > 9000 THEN
            'Powy¿ej 9k'
    END AS poziom_zarobków,
    salary
FROM
    hr.employees
ORDER BY
...
SELECT
    ...
    CASE
        WHEN round(months_between(DATE '2009-01-01', emp.hire_date)) < 13 THEN
            'Junior'
        WHEN round(months_between(DATE '2009-01-01', emp.hire_date)) >= 13
             AND round(months_between(DATE '2009-01-01', hire_date)) <= 40 THEN
            'Regular'
        WHEN round(months_between(DATE '2009-01-01', emp.hire_date)) > 40 THEN
            'Senior'
    END AS staz
FROM
    hr.employees emp
...
--B) z wykorzystaniem funkcji extract
SELECT
    hire_date,
...
--****Wyswietl wielkimi literami dwie pierwsze litery imienia pracownika
SELECT
    first_name,
...
--****Funkcja to_char konwertuje równiez liczby na napisy ****
SELECT
    to_char(129.111,'999D999L')AS Symbol_Waluty
FROM
    Dual;

--Zadanie Wyœwietl imiê, nazwisko i roczne zarobki pracowników uzwglêdniaj¹c, ¿e
--pracownicy dzia³u o ID 80 otrzymuj¹ premiê roczn¹ w wysokoœci 5000, a dzia³u o ID 50
--premiê roczn¹ w wysokoœci 3000.
--Wynik przestaw wed³ug formatu 288 000PLN.
